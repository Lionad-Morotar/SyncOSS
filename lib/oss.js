...
module.exports = function (options) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    let getObjects = function (cb) {
      ...
      let getObjectsLoop = function () {
        if (typeof nextMarker === 'string') {
          ...
          cb(bucketObjects)
          options.debug && console.log('Scan oss bucket finish, files count:', bucketObjects.length)
        }
      }
      getObjectsLoop()
    }
    ...
    getObjects(function (bucketObjects) {
      ...
      const tasker = new Tasker({ maxConcurrenceCount: 1 })
      ...
      let deleteFile = function (filePath) {
        ...
        const filename = standerPath.match(/[^\\\/]*$/)[0]
        oss.deleteObject(
          {
            Bucket: bucket.Name,
            Key: filename,
          ...
          function (err) {
            ...
            let bucketIndex = bucketPaths.indexOf(filename)
            if (bucketIndex !== -1) {
              bucketPaths.splice(bucketIndex, 1)
            }
            let localIndex = localPaths.indexOf(filename)
            if (localIndex !== -1) {
      ...
      filesWatcher.on('add', function (localFilePath) {
        ...
        if (bucketIndex === -1) {
          ...
          fs.readFile(localFilePath, function (readFileErr, fileData) {
            ...
            for (let i = 0; i < bucketObjects.length; i++) {
              if (bucketObjects[i].Key === filename) {
